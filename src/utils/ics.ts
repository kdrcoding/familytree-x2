...
export function buildFamilyCalendarIcs(
  ...
  const calendarName = options.calendarName ?? 'Shajira family';
  const now = new Date();
  ...
  const lines: string[] = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Shajira//Family Calendar//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
...
export function downloadFamilyCalendarIcs(
  ...
  a.href = url;
  a.download = options.filename ?? 'shajira-family-calendar.ics';
  a.click();
