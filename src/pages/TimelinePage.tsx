...
function EventIcon({ kind }: { kind: TimelineEvent['kind'] }) {
  ...
  switch (kind) {
    ...
    case 'memory':
      return <Camera className={`${className} text-brand-600`} aria-hidden />;
  }
}
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6 sm:px-6">
      ...
      {groups.length === 0 ? (
        ...
        <ol className="relative space-y-8 border-l-2 border-brand-200 pl-6 dark:border-brand-900">
          {groups.map(({ year, events: yearEvents }) => (
            <li key={year}>
              <h2 className="sticky top-16 z-10 -ml-[1.65rem] mb-4 inline-flex items-center gap-1.5 rounded-full bg-brand-700 px-3.5 py-1 text-sm font-bold text-white shadow-md shadow-brand-900/20">
                {year}
              </h2>
                ...
                {yearEvents.map((event) => {
                  ...
                  return (
                    <li key={event.id} className="card relative p-4 transition-shadow hover:shadow-md">
                      <span className="absolute -left-[1.9rem] top-5 flex h-5 w-5 items-center justify-center rounded-full bg-white ring-2 ring-brand-300 dark:bg-stone-950 dark:ring-brand-800">
