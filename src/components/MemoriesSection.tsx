...
function MemoryThumb({ memory, onOpen }: { memory: FamilyMemory; onOpen: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="group relative aspect-square overflow-hidden rounded-xl bg-stone-100 focus-visible:ring-2 focus-visible:ring-brand-600 dark:bg-stone-800"
...
export function MemoriesSection({ person }: MemoriesSectionProps) {
  ...
  return (
    <div className="mt-4 rounded-xl bg-stone-50 p-3 dark:bg-stone-800/60">
      ...
      {pendingDataUrl && (
        <div className="mt-3 space-y-2 rounded-xl border border-brand-200 bg-white p-3 dark:border-brand-900 dark:bg-stone-900">
          <img
            src={pendingDataUrl}
            alt=""
