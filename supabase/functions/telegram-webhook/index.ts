  ...
  try {
    ...
    if (member?.chat && (member.chat.type === 'group' || member.chat.type === 'supergroup')) {
      ...
      if (status === 'member' || status === 'administrator') {
        ...
        await sendText(
          member.chat.id,
          'Shajira birthday bot is ready for this group. Birthdays will be posted at the hour set in Settings.',
        );
      }
